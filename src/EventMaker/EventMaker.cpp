...
void EventMaker::updateSettings(const char* jsonString){
  //这里得到的字符串来自于http://10.155.101.208/upload 上传的文件内容
  //文件的第一行必须为 messageTo EventMaker, 例如：
  //  来自settings.json的内容截选：
  //   messageTo  EventMaker
  //{
  //  "WValue": 23.6,
  //  "Vdrift": 1,
  //  "FPC2": [
  ...
  //输出Electronic_Gain的值
  for(int i=0;i<32;i++){
      cout<<"FPC2["<<i<<"] "<<FPC2[i]<<endl;
  }
  cout<<"===========>>>>WValue: "<<WValue<<" Vdrift: "<<Vdrift<<endl;
}


Event* EventMaker::convert(const RawEvent &REvt){
    vector<Pad>().swap(event->pads);
    ...
    {   
        ...
        event->pads.push_back(pad);
    }
    return event;
}

void EventMaker::PadMap(int *XPos,int *ZPos){
    int IDD[200];
